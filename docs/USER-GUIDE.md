...
1. 任务状态为 Accepted
    ↓
2. 进入任务详情，Raise Dispute（发起争议）
    ↓
3. 任务状态变为 Disputed（信誉分暂不变化）
    ↓
4. 管理员（合约 Owner）进入 Wallet 页面
    ↓
5. 在 Resolve Dispute 卡片输入任务 ID 和裁决比例
    ↓
6. 点击 Resolve Dispute 解决争议
    ↓
7. 任务状态变为 Resolved，资金按比例分配
    ↓
8. 过错方信誉分扣除
```
